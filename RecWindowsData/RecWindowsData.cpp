...
DWORD WINAPI  UIThreadFunction(LPVOID lpParam)
{
    ...
    hAccelTable = LoadAccelerators(hInstance, MAKEINTRESOURCE(IDS_STRING_WINDOWS));
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.lpszMenuName = MAKEINTRESOURCE(IDS_STRING_WINDOWS);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case  WM_CREATE:
    {
        ...
        hWindowsTarget = CreateWindowEx(0,
            _T("STATIC"),
            NULL,
            WS_CHILD | WS_VISIBLE,
        ...
        SendMessage(hWindowsTarget, WM_SETFONT, (WPARAM)hFont, TRUE);
    }
    break;
    case WM_PAINT:
    ...
    case WM_COPYDATA:
    {
        ...
        _stprintf_s(szBuffer, _T("dwData:%d cbData:%d\r\nlpData:0x%08x = %s\r\n\r\n"), pCopyData->dwData, pCopyData->cbData, (PVOID)pCopyData->lpData, (char*)pCopyData->lpData);
        TCHAR szTemp[100];

        _stprintf_s(szTemp, _T("%s %%"), (LPCWSTR)(char*)pCopyData->lpData);

        if (!SetWindowText(hWindowsTarget, szTemp))
        {
            TCHAR str[78];
